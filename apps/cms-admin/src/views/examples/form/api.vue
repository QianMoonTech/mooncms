...
function handleClick(
  ...
  switch (action) {
    ...
    case 'componentRef': {
      ...
      formApi.getFieldComponentRef<SelectInstance>('fieldOptions')?.focus?.();
      break;
    }
    case 'disabled': {
...
<template>
  <Page description="表单组件api操作示例。" title="表单组件">
    <ElSpace class="mb-5 flex-wrap">
      ...
      <ElButton @click="handleClick('resetLabelWidth')">还原labelWidth</ElButton>
      ...
      <ElButton @click="handleClick('hiddenResetButton')">隐藏重置按钮</ElButton>
      <ElButton @click="handleClick('showResetButton')">显示重置按钮</ElButton>
      <ElButton @click="handleClick('hiddenSubmitButton')">隐藏提交按钮</ElButton>
      <ElButton @click="handleClick('showSubmitButton')">显示提交按钮</ElButton>
      <ElButton @click="handleClick('updateResetButton')">修改重置按钮</ElButton>
      <ElButton @click="handleClick('updateSubmitButton')">修改提交按钮</ElButton>
