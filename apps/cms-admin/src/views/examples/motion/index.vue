...
<template>
  <Page title="Motion">
    ...
    <ElCard class="mt-2" :body-style="{ padding: 0 }">
      <template #header>使用组件（将内部作为一个整体添加动画）</template>
      <div
      ...
      <div>
        <ElForm :model="motionProps" label-width="100px">
          <ElRow>
            <ElCol :span="8">
    ...
    <ElCard class="mt-2" :body-style="{ padding: 0 }">
      <template #header>分组动画（每个子元素都会应用相同的独立动画）</template>
      <div
        class="relative flex min-h-32 items-center justify-center gap-2 overflow-hidden"
      >
      ...
      <div>
        <ElForm :model="motionGroupProps" label-width="100px">
          <ElRow>
            <ElCol :span="8">
              <ElFormItem prop="preset" label="动画效果">
